...
interface Trade {
  id: string; date: string; pair: string; resultR: number | null; outcome: string; model: string; grade: string; session: string
}
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    fetch('/api/trades?limit=200').then(r => r.json()).then(d => {
  ...
  const equityCurve = trades
    ...
    .reduce((acc: { trade: number; r: number; cumR: number }[], t, i) => {
      ...
      acc.push({ trade: i + 1, r: t.resultR!, cumR: Math.round((prev + t.resultR!) * 100) / 100 })
      return acc
    }, [])
  ...
  if (!analytics || analytics.totalTrades === 0) {
    return (
      <div className="max-w-5xl mx-auto text-center py-20">
        ...
        <p className="text-gray-500 text-sm">No closed trades yet. Close some trades in the journal to see your statistics.</p>
      </div>
    )
  }
  ...
  const statCard = (label: string, value: string | number, sub?: string, color?: string) => (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <p className={`text-2xl font-semibold font-mono ${color || 'text-gray-900'}`}>{value}</p>
      {sub && <p className="text-xs text-gray-400 mt-1">{sub}</p>}
    </div>
  )

  const gradeColor = (g: string) => g === 'A+' ? '#1a6b4a' : g === 'B' ? '#d4830a' : '#888'

  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold">Strategy Analytics</h1>
        <p className="text-sm text-gray-500 mt-1">RFDM performance breakdown — {analytics.totalTrades} closed trades</p>
      </div>

      {/* Top stats */}
      <div className="grid grid-cols-3 gap-4 mb-8 md:grid-cols-6">
        {statCard('Win Rate', `${analytics.winRate}%`, `${analytics.wins}W / ${analytics.losses}L`,
          analytics.winRate >= 55 ? 'text-green-700' : analytics.winRate >= 45 ? 'text-amber-700' : 'text-red-700')}
        {statCard('Total R', `${analytics.totalR > 0 ? '+' : ''}${analytics.totalR}R`, 'cumulative profit',
          analytics.totalR > 0 ? 'text-green-700' : 'text-red-700')}
        {statCard('Avg R/Trade', `${analytics.avgR > 0 ? '+' : ''}${analytics.avgR}R`, 'per closed trade')}
        {statCard('Model A', `${analytics.modelA.winRate}%`, `${analytics.modelA.trades} trades`)}
        {statCard('Model B', `${analytics.modelB.winRate}%`, `${analytics.modelB.trades} trades`)}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-8">
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={equityCurve}>
                <XAxis dataKey="trade" tick={{ fontSize: 11, fill: '#888' }} label={{ value: 'Trade #', position: 'insideBottom', offset: -2, fontSize: 11, fill: '#888' }} />
                <YAxis tick={{ fontSize: 11, fill: '#888' }} />
                <Tooltip formatter={(v: number) => [`${v > 0 ? '+' : ''}${v}R`, 'Cumulative R']} labelFormatter={l => `Trade #${l}`} />
                <Line type="monotone" dataKey="cumR" stroke={equityCurve[equityCurve.length - 1]?.cumR >= 0 ? '#1a6b4a' : '#c0392b'}
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <div className="space-y-3 mt-2">
            {analytics.bySession.filter(s => s.trades > 0).map(s => (
              <div key={s.session}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-700">{s.session}</span>
                  <span className="font-mono font-medium">{s.winRate.toFixed(0)}% <span className="text-gray-400 font-normal">({s.trades} trades)</span></span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all"
                    style={{ width: `${s.winRate}%`, background: s.winRate >= 55 ? '#1a6b4a' : s.winRate >= 45 ? '#d4830a' : '#c0392b' }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={analytics.byGrade.filter(g => g.trades > 0)} barSize={40}>
              ...
              <Bar dataKey="winRate" radius={[4, 4, 0, 0]}>
                {analytics.byGrade.map(g => <Cell key={g.grade} fill={gradeColor(g.grade)} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <div className="mt-3 space-y-1">
            {analytics.byGrade.filter(g => g.trades > 0).map(g => (
              <div key={g.grade} className="flex justify-between text-xs">
                <span className="text-gray-500">{g.grade} — {g.trades} trades</span>
                <span className="font-mono">{g.totalR > 0 ? '+' : ''}{g.totalR.toFixed(1)}R total</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-8">
        ...
        <div className="grid grid-cols-2 gap-6">
          {[
            { label: 'Model A — Wyckoff trap', ...analytics.modelA, color: '#1a6b4a', desc: 'Spring / upthrust entry' },
            { label: 'Model B — Liquidity run', ...analytics.modelB, color: '#d4830a', desc: 'Retest of broken structure' },
          ].map(m => (
            <div key={m.label} className="text-center">
              <p className="text-sm font-medium text-gray-700 mb-1">{m.label}</p>
              <p className="text-xs text-gray-400 mb-4">{m.desc}</p>
              <p className="text-4xl font-mono font-semibold" style={{ color: m.color }}>{m.winRate}%</p>
              <p className="text-xs text-gray-400 mt-1">win rate · {m.trades} trades</p>
            </div>
          ))}
        </div>
