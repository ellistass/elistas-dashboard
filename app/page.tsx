...
function ScoreBadge({ score }: { score: number }) {
  const cls =
    score > 0 ? "text-green-700" : score < 0 ? "text-red-600" : "text-gray-400";
  return (
...
export default function Dashboard() {
  ...
  async function runManual(sendAlert = false) {
    ...
    try {
      ...
      if (!res.ok) {
        setScoreStatus({
          ok: false,
          msg: json.error || "Manual scoring failed",
        });
      } else {
        setData((prev) =>
          prev
  ...
  const currentSession =
    watHour >= 1 && watHour < 7
      ? "Tokyo"
      : watHour >= 8 && watHour < 11
        ? "London"
        : watHour >= 13 && watHour < 14
          ? "Pre-NY"
          : watHour >= 15 && watHour < 18
            ? "New York"
            : null;
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {scoreStatus && (
        <div
          className={`mb-4 px-4 py-3 rounded-xl text-xs font-mono flex items-center justify-between ${
            scoreStatus.ok
        ...
        >
          ...
          <button
            onClick={() => setScoreStatus(null)}
            className="ml-4 opacity-50 hover:opacity-100 text-sm"
          >
            ×
          </button>
        </div>
      ...
      {data?.fetchErrors && data.fetchErrors.length > 0 && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800">
          ...
          {!data.hasLiveData && (
            <span className="ml-2">
              —{" "}
              <button onClick={() => setShowManual(true)} className="underline">
                enter data manually
              </button>
            </span>
          )}
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p
                      className={`text-xs font-medium mt-0.5 ${trade.direction === "Short" ? "text-red-600" : "text-green-700"}`}
                    >
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
                  ...
                  {trade.divScore && (
                    <span className="text-xs text-gray-400 ml-auto">
                      div {trade.divScore.toFixed(1)}
                    </span>
                  )}
                </div>
                {trade.alignmentStatus !== "Green" &&
                  trade.alignmentStatus !== "Unknown" && (
          ...
          <div className="space-y-1.5">
            {SESSION_WINDOWS.map((s) => {
              ...
              return (
                <div
                  key={s.name}
                  className={`rounded-xl px-3 py-2.5 flex items-center justify-between text-sm border transition-all ${
                    isActive
                ...
                >
                  ...
                  {s.prime && (
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${isActive ? "bg-green-200 text-green-800" : "bg-gray-100 text-gray-400"}`}
                    >
                      prime
                    </span>
                  )}
              ...
              {scoring ? (
                <>
                  ...
                  <p className="text-sm text-gray-600 font-medium">
                    Claude is analysing the markets…
                  </p>
                  <p className="text-xs text-gray-400 mt-1">
                    Fetching data → Scoring currencies → Building matrix
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-xs text-gray-400 mt-1 mb-4">
                    Tap Run Analysis to score today&apos;s market with Claude AI
                  </p>
                  <button
                    onClick={() => runAnalysis(false)}
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  ...
                  <div className="space-y-2">
                    ...
                    {scores.top3.map((c, i) => (
                      <div
                        key={c.cur}
                        className="card-strong flex items-center justify-between"
                      >
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-green-600 opacity-60">
                            #{i + 1}
                          </span>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-green-600 mt-0.5 max-w-32 truncate">
                            {c.tag}
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {scores.bottom3.map((c, i) => (
                      <div
                        key={c.cur}
                        className="card-weak flex items-center justify-between"
                      >
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-red-500 opacity-60">
                            #{i + 1}
                          </span>
                        ...
                        <div className="text-right">
                          <ScoreBadge score={c.score} />
                          <p className="text-xs text-red-500 mt-0.5 max-w-32 truncate">
                            {c.tag}
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="bg-black text-white rounded-2xl p-6">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div>
                        <p className="font-mono text-2xl font-semibold tracking-tight">
                          {scores.priority1.pair}
                        </p>
                        <p className="text-gray-400 text-sm mt-1">
                          {scores.priority1.direction} ·{" "}
                          {scores.priority1.strong} vs {scores.priority1.weak}
                        </p>
                        <div className="flex items-center gap-2 mt-3">
                          <GradeBadge grade={scores.priority1.grade} />
                          <span className="text-xs text-gray-500">
                            {scores.priority1.session?.join(" · ")}
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      {[
                        ...
                        "Declare A or B",
                        "Min 1:2 R:R",
                        "No entry 30m after open",
                      ].map((r) => (
                        <span
                          key={r}
                          className="text-xs px-2.5 py-1 bg-gray-800 text-gray-400 rounded-full"
                        >
                          {r}
                        </span>
                      ))}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-100">
                        ...
                        {scores.bottom3.map((w) => (
                          <th
                            key={w.cur}
                            className="p-3 text-xs font-mono font-semibold text-red-600"
                          >
                            {w.cur}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {scores.top3.map((s) => (
                        ...
                        >
                          <td className="p-3 font-mono text-sm font-semibold text-green-700">
                            {s.cur}
                          </td>
                          {scores.bottom3.map((w) => {
                            const p = scores.pairs9.find(
                              (x) => x.strong === s.cur && x.weak === w.cur,
                            );
                            if (!p)
                              return (
                                <td
      ...
      <div className="mt-8 border-t border-gray-200 pt-6">
        ...
        >
          <span
            className={`transition-transform text-xs ${showManual ? "rotate-90" : ""}`}
          >
            ▶
          </span>
          ...
          <div className="mt-4 bg-white border border-gray-200 rounded-2xl p-5">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={scoring || (!perf.trim() && !calendar.trim())}
