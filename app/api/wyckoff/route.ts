...
export async function GET() {
  ...
  try {
    // Unresolved: open ranges first (decision is live), then broken ranges
    // still inside their 12-bar resolution window.
...
export async function POST() {
  ...
  try {
    const { runWyckoffScan } = await import("@/lib/wyckoff/scan");
